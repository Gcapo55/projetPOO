"""
Classes Texte (modèle d'un texte littéraire avec titre, auteur, contenu et année)
et ChargeurTexte (lecture et parsing d'un fichier .txt au format
Project Gutenberg depuis ./docs/).
"""
...
class ChargeurTexte:
    ...
    def charger(self, source: str) -> Texte:
        with Path(BASE_DIR / "docs" / source).open("r", encoding="utf-8") as file:
            ...
            contenu_match = re.search(
                r"\*\*\*[^*]+\*\*\*(.+?)(?=\*\*\*|$)", global_contenu, re.DOTALL
            )  # noqa: E501
            contenu = contenu_match.group(1) if contenu_match else "Contenu Inconnu"
            date_match = re.search(
                r"(?<=Original Publication:\s).+?([\d]{4})(?=\s|\n)", global_contenu
            )  # noqa: E501
            date = "Inconnue"
