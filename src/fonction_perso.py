"""
Fonctions d'analyse morphosyntaxique via spaCy pour extraire
les attributs adjectivaux d'un personnage (les 3 plus fréquents)
et déterminer son genre grammatical à partir d'un Doc.
"""
...
def check_attribut(n: str, tok: Token, doc: Doc) -> str | None:
    ...
    if tok.text not in nom and tok.head.text not in nom:
        return None
    if tok.text in nom:
        if (tok.dep_ == "appos" and tok.head.pos_ == "ADJ") or (
            tok.dep_ == "nsubj" and tok.head.pos_ == "ADJ"
        ):
            return tok.head.lemma_

        if tok.dep_ == "appos" and doc[tok.i - 1].pos_ == "ADJ":
            return doc[tok.i - 1].lemma_

        return None

    if tok.head.text in nom and tok.dep_ == "amod":
        return tok.lemma_
